...
dailyCaloriesAnalysis <- function(fileNameInput){
    ...
    curPlot <- ggplot(data=aggFrame, aes(x=dfTime, )) + 
        ...
        geom_line(aes(y=rollmean(AvgCalories, 7, na.pad=TRUE), color=AvgCalories)) +
        geom_line(aes(y=rollmean((SumCalories / KCal), 7, na.pad=TRUE), color=(SumCalories / KCal))) +
        scale_color_viridis( option = "D")+
        ylim(-50, 5050) + 
        scale_fill_viridis()  +
        #geom_smooth(aes(x=dfTime, y=AvgCalories, color='blue'), method = "lm") +
        #geom_smooth(aes(x=dfTime, y=SumCalories / KCal, color='red'), method = "lm") +
        scale_x_date(date_breaks = "1 week", date_labels = "%D") +
        scale_y_continuous(
    ...
    curPlot <- ggplot(data=mergeDF, aes(x=dfTime, y=Calories, group=Id)) + 
        ...
        aes(color=Id) +
        geom_point() + 
        geom_smooth(aes(x=dfTime, y=Calories, color=Id), method = "loess") +
        ylim(-50, 5050) + 
